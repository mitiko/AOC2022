...
best = 0
def search(curr_best, curr, time_left, mask, elephant):
    global best
    if curr_best > best:
        best = curr_best
    ...
    if not is_set(mask, curr):
        if not elephant:
            search(curr_best + rates[curr] * time_left, 'AA', 25, set_bit(mask, curr), True)
        search(curr_best + rates[curr] * time_left, curr, time_left - 1, set_bit(mask, curr), elephant)
        return

    for x in arr:
        if is_set(mask, x): continue
        search(curr_best, x, time_left - dist[curr,x], mask, elephant)

search(0, 'AA', 26, 0, False)
print(best)
